...
?>
<head>
  <title>DENR-R8 DVTS | Pending DV</title>
  ...
  <div class="content-wrapper">
    ...
    <section class="content-header">
      <div class="container-fluid">
        <div class="row mb-2">
          <div class="col-sm-6">
            <h1>Pending DV Table</h1>
          </div>
          <div class="col-sm-6">
            <ol class="breadcrumb float-sm-right">
              <li class="breadcrumb-item"><a href="#">Home</a></li>
              <li class="breadcrumb-item active">Pending DV Table</li>
            </ol>
          </div>
        </div>
      </div><!-- /.container-fluid -->
            ...
            <div class="card-header">
                        ...
                        <div class="modal-body">
                            <div class="row">
                                ...
                                <div class="form-group col-md-4">
                                    ...
                                    <?php 
                                    $sql = "SELECT user_id, fullname, department_id FROM tbl_admin WHERE user_role = 2 and 3 ORDER BY fullname ASC";
                                    $info = $obj_admin->manage_all_info($sql);   
                                    ?>
                ...
                <div class="col-12 col-sm-12">
                    ...
                    <ul class="nav nav-tabs" id="custom-tabs-one-tab" role="tablist">
                        <li class="nav-item">
                        <a class="nav-link active" href="pending-dv.php">Pending DV's</a>
                        </li>
                        <li class="nav-item">
                        <a class="nav-link" href="created-dv.php">Created DV's</a>
                        </li>
                        <li class="nav-item">
                        ...
                        <li class="nav-item">
                        <a class="nav-link" href="for-fund-transfer-dv.php">For Findes DV's</a>
                        </li>
                        <li class="nav-item">
                        ...
                        <div class="table-responsive">
                            ...
                            <?php 
                                if ($user_role == 1) {
                                    ...
                                    LEFT JOIN tbl_admin assignee ON a.t_user_id = assignee.user_id
                                    LEFT JOIN payee c ON a.payee_sms_id = c.payee_sms_id
                                    WHERE a.t_department_id = $department_id AND a.status NOT IN (3, 5, 6)
                                    ORDER BY a.task_id DESC";
                                }
                                $info = $obj_admin->manage_all_info($sql);
                                $serial  = 1;
                                ...
                                <tr>
                                    ...
                                    <td>
                                        <?php  if($row['status'] == 1){
                                            ...
                                            echo "<span class='badge bg-success'><i class='fas fa-check-circle'></i> Complied</span>";
                                            }elseif($row['status'] == 5){
                                            echo "<span class='badge bg-success'><i class='fas fa-check-circle'></i> For Payment</span>";
                                            }elseif($row['status'] == 6){
                                            echo "<span class='badge bg-success'><i class='fas fa-check-circle'></i> For Certification</span>";
                                            }elseif($row['status'] == 7){
                                            echo "<span class='badge bg-success'><i class='fas fa-check-circle'></i> For Approval</span>";
                                            }elseif($row['status'] == 8){
                                            echo "<span class='badge bg-success'><i class='fas fa-check-circle'></i> For Fund Transfer</span>";
                                            }elseif($row['status'] == 9){
                                            echo "<span class='badge bg-success'><i class='fas fa-check-circle'></i> For Actual Credit</span>";
                                        }else{
                                    ...
                                    <td>
                                        <div class="action-buttons">
                                            ...
                                            <?php } elseif ($user_role == 3 || $_SESSION['department_id'] == 2) { ?>
                                                <button class="btn btn-block btn-outline-success"
                                                    title="approve" data-toggle="modal" data-target="#modal-success" data-taskid="<?php echo $row['task_id'];?>" data-task-desc="<?php echo $row['t_description']; ?>"
                      data-payee-sms-id="<?php echo $row['payee_sms_id']; ?>" data-taskid="<?php echo $row['task_id'];?>">
                                                    <i class="nav-icon fas fa-thumbs-up"></i>
                                                </button>
                                                <button class="btn btn-block btn-outline-danger"data-taskid="<?php echo $row['task_id'];?>" data-task-desc="<?php echo $row['t_description']; ?>"
                      data-payee-sms-id="<?php echo $row['payee_sms_id']; ?>"
                                                    title="Disapprove" data-toggle="modal" data-target="#modal-danger">
...
<div class="modal fade" id="modal-danger">
    <div class="modal-dialog">
        <form method="POST" action="disapprove-task.php">
            <div class="modal-content bg-danger">
                <div class="modal-header">
                    <h4 class="modal-title" id="disapproveModalLabel">Disapprove Task</h4>
                </div>
                <div class="modal-body">
                    <input type="hidden" name="task_id" id="modal_dis_task_id">
                    <input type="hidden" name="action" value="disapprove">
                </div>
                <div class="form-group">
                    <label for="disapprove_reason">Reason for Non-Approval:</label>
                    <textarea class="form-control" name="comment" id="disapprove_reason" rows="4" required></textarea>
                </div>
...
$('modal-danger').on('show.bs.modal', function (event) {
  // 1) Who triggered the modal:
  var button = $(event.relatedTarget);

  // 2) Read the data-taskid attribute:
  var taskId = button.data('taskid');

  // 3) Set it in the hidden input:
  $(this).find('#modal_dis_task_id').val(taskId);

  // (optional) Also attach it to the submit button itself:
  $(this).find('button[type=submit]')
         .attr('data-task-id', taskId);
});
</script>
<!-- Approve Modal -->
<div class="modal fade" id="modal-success" aria-labelledby="approveModalLabel">
    <div class="modal-dialog">
        <form method="POST" action="approve-task.php">
            <div class="modal-content bg-success">
                <div class="modal-header">
                    <h4 class="modal-title" id="approveModalLabel">Approve Disbursement Voucher</h4>
                </div>
                <div class="modal-body">
                    <input type="hidden" name="task_id" id="modal_task_id">
                    <input type="hidden" name="action" value="approve">
                    <p>Are you sure with your action?<br>If yes, click <strong>Proceed</strong> to continue. An email notification will be sent to the claimant about the status of this voucher.</p>
                </div>
                <div class="modal-footer">
                    <button type="button" class="btn btn-outline-dark" data-dismiss="modal">Cancel</button>
                    <button type="submit" class="btn btn-outline-light">Proceed</button>
                </div>
            </div>
        <!-- /.modal-content -->
        </form>
...
$('#modal-success').on('show.bs.modal', function (event) {
  ...
  // 3) Set it in the hidden input:
  $(this).find('#modal_task_id').val(taskId);
...
$(document).ready(function () { 
    $(".btn-outline-light, .btn-outline-dark").click(function () {
